 ...
 * ConnectionPool connPool = new ConnectionPool("com.microsoft.jdbc.sqlserver.SQLServerDriver"
 ,"jdbc:microsoft:sqlserver://localhost:1433;DatabaseName=MyDataForTest","Username","Password");
 * ②　
 * connPool .createPool();
...
public class ConnectionPool {
    ...
    private String testTable = "U_USER"; // 测试连接是否可用的测试表名，默认没有测试表
     ...
     * incrementalConnections 设置 的值创建几个数据库连接，并放入连接池中。 如果创建后，所有的连接仍都在使用中，
     * 则返回 null
     *
     * @return 返回一个可用的数据库连接
     */
